...
from environment.env_comparison.GridCells import GridCells
...
class ZeroTwentyPCBBoard(pcb.PCBBoard):
    ...
    def is_valid_move(self, agent, action):
        ...
        if action % 2 == 0:
            ...
            if self.grid[r + delta_r, 0] == a1 or self.grid[0, c + delta_c] == a2:
                return False
    ...
    def padded_observation(self, agent_id):
        ...
        grid_observation = np.ones(shape=(self.obs_rows, self.obs_cols), dtype=np.float32)

        grid_observation[row_start_delta:self.obs_rows + row_end_delta, col_start_delta:self.obs_cols + col_end_delta] \
            = self.grid[row_start:row_end, col_start:col_end] / len(GridCells)
